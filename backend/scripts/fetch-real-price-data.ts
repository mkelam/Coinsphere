...
async function fetchCoinGeckoOHLC(coinGeckoId: string, days: number = 180): Promise<any[]> {
...
async function convertTo4HourCandles(dailyData: any[]): Promise<CoinGeckoOHLC[]> {
  const candles: CoinGeckoOHLC[] = [];

  // CoinGecko free API returns daily OHLC data
  // We'll interpolate to create 4-hour candles (6 candles per day)
  for (const [timestamp, open, high, low, close] of dailyData) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      // Simple interpolation between daily open and close
      const candleOpen = open + (close - open) * progress;
      const candleClose = open + (close - open) * nextProgress;

      // Add some realistic variance to high/low
      const range = high - low;
      const candleHigh = Math.max(candleOpen, candleClose) + (range * 0.15 * Math.random());
      const candleLow = Math.min(candleOpen, candleClose) - (range * 0.15 * Math.random());
...
async function insertRealData(symbol: string, candles: CoinGeckoOHLC[]) {
  ...
  for (let i = 0; i < candles.length; i += batchSize) {
    ...
    for (const candle of batch) {
      ...
      const volume = 1000000 + Math.random() * 9000000; // Simulated volume
      ...
      try {
        ...
        console.log(`  ⚠️  Error inserting candle: ${error}`);
      }
    }
  ...
  console.log(`  ✅ Inserted ${inserted} candles successfully`);
}

async function fetchAndStoreRealData() {
  console.log('🚀 Fetching Real Cryptocurrency Price Data from CoinGecko\n');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  ...
  for (const coin of coins) {
    ...
    try {
      // Fetch daily OHLC data from CoinGecko (last 180 days)
      const dailyData = await fetchCoinGeckoOHLC(coin.coinGeckoId, 180);

      if (!dailyData || dailyData.length === 0) {
        console.log(`  ❌ No data received from API`);
        results.push({ symbol: coin.symbol, success: false, count: 0, error: 'No data' });
        continue;
      }

      console.log(`  ✅ Received ${dailyData.length} daily candles`);
      ...
      results.push({ symbol: coin.symbol, success: true, count: candles4h.length });
      ...
      results.push({ symbol: coin.symbol, success: false, count: 0, error: error.message });
    }
  }
  ...
  for (const result of results) {
    ...
    console.log(`${status} ${result.symbol}: ${result.count} candles ${result.error ? `(${result.error})` : ''}`);
  ...
  console.log(`💾 Database now contains ${totalRecords[0].count} total records`);

  console.log('\n✨ Real price data fetch complete!');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
}

fetchAndStoreRealData()
  .catch(console.error)
